...
def author():                                                                                             
        return "icanlapan3" # replace gburdell3 with your Georgia Tech username.
                                                                                              
def gtid():                                                                                               
    return 903565329 # replace with your GT ID number
...
class PageRank:
    ...
    def calculate_node_degree(self):
        ...
        #############################################
            ...
            # Add or update degree in node_degree dictionary
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        for it in range(iterations):
            ...
            for source, target in self.read_edge_file(self.edge_file):
                ...
                out_d = self.node_degree[source]
                
                # Get page rank of source
                pg_rank = pr_values[source]
                
                # Calculate the fraction
                new_pr_values[target] += (pg_rank/out_d)
           
            weight = (1-damping_factor)*node_weights
            pr_values = weight + [x * damping_factor for x in new_pr_values]
...
#                 source_prop = pr_values[source]/self.node_degree[source]
#                 new_pr_values[target] += damping_factor*source_prop

#             weight = (1 - damping_factor) * node_weights
#             new_pr_values = np.add(weight, new_pr_values)
