...
var y_log = d3.scaleLog().clamp(true).range([height, 0]);
...
var svg4 = d3.select("#Ratings_Rankings_LOG").append("svg")
...
// Scale the range of the data
      ...
      y_log.domain([1, d3.max(finalData, function(d) { return d.count; })]);
...
// Loop through data to add lines
        ...
        dataNest.forEach(function(d) {
            ...
            svg4.append("text")
                ...
                return d.color = color(d.key); })
                .text(d.key)
                .attr("transform", "translate(" + (width+3) + "," + y_log(d.values[d.values.length-1].count) + ")")

            if (game_name == "Catan" | game_name == "Codenames" | game_name == "Terraforming Mars" | game_name == "Gloomhaven") {
                // Add dots

                var g = svg4.selectAll(null)
                            .data(d.values)
                            .enter()
                            .append("g")
